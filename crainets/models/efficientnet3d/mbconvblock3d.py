...
class MBConvBlock3D(nn.Module):
    ...
    def __init__(self,
        ...
        Conv3d = Conv3dDynamicSamePadding

        if self.expand_ratio != 1:
            self.expand_conv = Conv3d(in_channels=in_channels,
        ...
        if self.has_se:
            ...
            num_squeezed_channels = max(1, se_ratio*self.in_channels/self.mid_channels)
            self.se_reduce = Conv3d(in_channels=self.mid_channels, out_channels=num_squeezed_channels, kernel_size=1)
            self.se_expand = Conv3d(in_channels=num_squeezed_channels, out_channels=self.mid_channels, kernel_size=1)


        # Pointwise convolution phase
        self.project_conv = Conv3d(in_channels=self.mid_channels,
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.has_se:
            ...
            x_squeezed = self.silu(x_squeezed)
